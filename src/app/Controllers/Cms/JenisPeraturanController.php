...
class JenisPeraturanController extends BaseAdminController
{
    use ResponseTrait;
    protected $var = [];
    protected $apiDomain;
    ...
    {
        $this->var['viewPath'] = 'cms/jenis_peraturan/';
        $this->apiDomain = getenv('API_DOMAIN');
        $this->titleHeader = 'Jenis Peraturan';
    }
    ...
    public function create()
    {
        ...
        if ($response->getStatusCode() == 201) {
            return redirect()->to('/admin/jenis_peraturan/index')->with('success', 'Data berhasil disimpan.');
        } else {
    ...
    public function edit($id = null)
    {
        ...
        if ($id) {
            $dataRequest = [
                'method' => 'GET',
                'api_path' => '/api/jenis_peraturan/edit/' . $id,
            ];
            $response = $this->request($dataRequest);
            if ($response->getStatusCode() == 200) {
    ...
    public function update($id = null)
    {
        ...
        $dataRequest = [
            'method' => 'POST',
            'api_path' => '/api/jenis_peraturan/update/' . $id,
            'form_params' => [
                'nama' => $nama,
            ],
        ];
    ...
    public function delete($id = null)
    {
        ...
        if ($id) {
            $dataRequest = [
                'method' => 'POST',
                'api_path' => '/api/jenis_peraturan/delete/' . $id,
            ];
